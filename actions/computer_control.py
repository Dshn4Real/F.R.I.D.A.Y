...
def _safe_screenshot_path(requested: str | None) -> Path:
    fallback = Path.home() / "Desktop" / "FRIDAY_screenshot.png"
    if not requested:
        return fallback
    try:
...
def _user_profile() -> dict:
    ...
    try:
        ...
        identity = load_memory().get("identity", {}) or {}
        out = {}
...
def _screen_find(description: str) -> tuple[int, int] | None:
    """Vision-grounded element lookup: screenshot → Gemini → click coordinates.

    Two traps this now handles:
      * Gemini vision models are trained to answer in a NORMALIZED 0–1000
        coordinate space. The old prompt asked for raw pixels and trusted the
        answer, so clicks drifted proportionally to how non-square the screen
        was. We now ask for the space the model natively thinks in and scale.
      * Under Windows DPI scaling, pyautogui.screenshot() returns the
        physical-pixel image while pyautogui.click() expects logical
        coordinates — so coordinates must be scaled to pyautogui.size(),
        never to the image size."""
    ...
    try:
        ...
        _require_pyautogui()
        scr_w, scr_h = pyautogui.size()          # logical (click) space
        img   = pyautogui.screenshot()
        buf   = io.BytesIO()
        ...
        prompt = (
            f"Locate the UI element described as: '{description}' in this "
            f"screenshot. Reply with ONLY the CENTER of the element as "
            f"normalized coordinates in a 0-1000 space, formatted exactly: x,y "
            f"(e.g. 512,377). If the element is not visible, reply: NOT_FOUND"
        )
        ...
        if match:
            ...
            if nx > 1000 or ny > 1000:
                # Model ignored the normalized request and answered in image
                # pixels — scale from the screenshot's physical size instead.
                x = round(nx * scr_w / img.width)
                y = round(ny * scr_h / img.height)
            else:
                x = round(nx * scr_w / 1000)
                y = round(ny * scr_h / 1000)
            return (min(max(x, 0), scr_w - 1), min(max(y, 0), scr_h - 1))
...
def _uia_click(name: str) -> str | None:
    """Accessibility-tree click: find a control by visible name in the
    foreground window via UI Automation and click it. Far more reliable than
    pixel-guessing when the app exposes its widgets — exact hit point, no
    resolution or theme sensitivity. Returns None when UIA cannot find it,
    so the caller can fall back to vision."""
    if platform.system() != "Windows":
        return None
    try:
    ...
    try:
        ...
        # Exact title first, then substring — a button called "Save" must not
        # lose to "Save As…" on an exact request.
        for kwargs in (
            {"title": name},
            {"title_re": f"(?i).*{re.escape(name)}.*"},
        ):
            try:
                ctrl = win.child_window(**kwargs, found_index=0)
                ctrl.wait("visible", timeout=2)
                ctrl.click_input()
                return f"Clicked '{name}' via UI Automation."
            except Exception:
                continue
    except Exception as e:
        print(f"[ComputerControl] UIA lookup failed: {e}")
    return None


def _verify_screen(question: str) -> str:
    """Look at the screen and answer a question about it — the agent loop's
    eyes for checking that a step actually did what it claimed."""
    api_key = _get_api_key()
    if not api_key:
        return "Cannot verify: no API key."
    try:
        from google import genai
        from google.genai import types as gtypes
        ...
        response = client.models.generate_content(
            ...
            contents=[
                ...
                (
                    "Answer this question about the screenshot in ONE short "
                    f"sentence, starting with YES or NO if applicable: {question}"
                ),
            ],
        )
        return (response.text or "").strip()[:300] or "No answer."
    except Exception as e:
        return f"verify_screen failed: {e}"
...
def computer_control(
    ...
    Actions:
      ...
      ui_click      — accessibility-tree click by element name (UIA first,
                      vision fallback) — most reliable way to press a button
      verify_screen — look at the screen and answer a question about it
    ...
    try:
        ...
        if action == "ui_click":
            ...
            if not desc:
                return "ui_click needs a description (the element's visible name)."
            hit = _uia_click(desc)
            if hit:
                return hit
            # Not in the accessibility tree (canvas-drawn UI, games, some
            # Electron apps) — fall back to the vision finder.
            coords = _screen_find(desc)
            if coords:
                time.sleep(0.2)
                _click(x=coords[0], y=coords[1])
                return f"Clicked '{desc}' at {coords} (vision fallback)."
            return f"Could not find '{desc}' via UI Automation or on screen."

        if action == "verify_screen":
            q = params.get("description", "") or params.get("text", "")
            if not q:
                return "verify_screen needs a question about the screen."
            return _verify_screen(q)
