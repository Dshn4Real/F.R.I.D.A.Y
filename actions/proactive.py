...
class ProactiveEngine:
    ...
    Improvements over 1.0:
      ...
      - Smarter silence gate    (doesn't fire while FRIDAY is speaking)
    ...
    def build_prompt(
        ...
        if followups:
            follow_ctx = (
                "\nOpen follow-ups that are due now:\n"
                + "\n".join(f"- {f}" for f in followups[:4])
                + "\nYou may mention one if it would actually help."
            )
