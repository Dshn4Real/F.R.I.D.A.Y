...
def _detect_default_browser() -> str:
    # Friday 4.0 refinement: honour preferred_browser from config (e.g. operagx)
    try:
        from memory.config_manager import load_api_keys
        pref = str((load_api_keys() or {}).get("preferred_browser") or "").strip().lower()
        if pref:
            aliases = {
                "opera gx": "operagx", "opera_gx": "operagx", "opera": "operagx",
                "msedge": "edge", "google chrome": "chrome",
            }
    ...
    try:
        if _OS == "Windows":
            ...
            winreg.CloseKey(k)
            for kw in ("operagx", "opera", "edge", "firefox", "brave", "vivaldi", "chrome"):
                if kw in prog_id:
                    return "operagx" if kw == "opera" and "gx" in prog_id else kw
        elif _OS == "Darwin":
            out = subprocess.run(
                ["defaults", "read",
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            ...
            try:
                ...
                friday_profile = str(Path.home() / ".friday_profiles" / "firefox_friday")
        ...
        # Real profile unavailable (browser already open / locked / blocked).
        # Fall back to a persistent Friday automation profile.
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "FRIDAY_screenshot.png")
