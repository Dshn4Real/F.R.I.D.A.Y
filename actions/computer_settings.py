...
# Tests inject a fake Windows endpoint so we never touch real speakers.
# Expected methods: get_scalar() -> float 0-1, set_scalar(f), get_mute() -> bool,
# set_mute(bool).
_volume_backend = None
...
def _parse_volume_value(value, default: int | None = None) -> int | None:
    """Gemini often sends value as the string '50' or '50%'."""
...
def _explicit_volume_target(text: str) -> int | None:
    """Number the user named as a destination ('to 20', 'limit 30'), not a step."""
    low = (text or "").lower()
    for rx in (
        r"\b(?:to|at)\s+(\d{1,3})\s*%?",
        r"\blimit(?:\s+(?:to|at))?\s+(\d{1,3})\s*%?",
        r"\bmake it\s+(\d{1,3})\s*%?",
        r"\bset(?:\s+(?:the\s+)?(?:volume|sound))?\s+(?:to\s+|at\s+)?(\d{1,3})\s*%?",
    ):
        m = re.search(rx, low)
        if m:
            return max(0, min(100, int(m.group(1))))
...
# Description beats a wrong action name. Gemini often sends volume_up
# (listed first in the tool schema) even when the user said "decrease".
_VOL_DOWN_RE = re.compile(
    r"\b(down|decrease|decreased|decreasing|lower|lowering|quieter|"
    r"reduce|reduced|reducing|softer|quiet)\b"
)
_VOL_UP_RE = re.compile(
    r"\b(up|increase|increased|increasing|louder|raise|raised|raising|higher)\b"
)
_VOL_BY_RE = re.compile(r"\bby\b")
_VOL_TO_RE = re.compile(r"\b(to|at)\b")
_VOL_SET_RE = re.compile(r"\b(set|make it)\b")
_VOL_ACTION_ALIASES = {
...
def _resolve_volume_intent(action: str, description: str, value) -> tuple[str, int | None]:
    """Pick volume_up / volume_down / volume_set from action + what they said.

    The Live model frequently sends action=volume_up with
    description='decrease the volume', or volume_set with the number only
    in description. Trust the words over the action name.
    """
    desc = (description or "").strip()
    low = desc.lower()
    action = (action or "").strip().lower().replace(" ", "_").replace("-", "_")
    action = _VOL_ACTION_ALIASES.get(action, action)

    explicit = _explicit_volume_target(desc)
    value_number = _parse_volume_value(value, default=None)
    # Words win over a hallucinated `value` like 50 when they said "to 20".
    number = explicit if explicit is not None else value_number
    if number is None:
        number = _parse_volume_value(desc, default=None)

    down = bool(_VOL_DOWN_RE.search(low))
    up = bool(_VOL_UP_RE.search(low))
    by_amt = bool(_VOL_BY_RE.search(low))
    to_amt = bool(
        explicit is not None
        or _VOL_TO_RE.search(low)
        or _VOL_SET_RE.search(low)
    )

    if down and not up:
        if number is not None and to_amt and not by_amt:
            return "volume_set", number
        return "volume_down", number
    if up and not down:
        if number is not None and to_amt and not by_amt:
            return "volume_set", number
        return "volume_up", number

    if number is not None and (
        to_amt or action in ("volume_set", "volume", "set")
    ):
        return "volume_set", number
    if action == "volume" and number is not None:
        return "volume_set", number
...
def _looks_like_volume(action: str, description: str) -> bool:
    blob = f"{action} {description}".lower()
    return (
        (action or "") in _VOL_ACTION_ALIASES
        or (action or "").startswith("volume")
        or (action or "") in ("mute", "unmute", "toggle_mute")
        or any(w in blob for w in ("volume", "mute", "unmute", "sound"))
    )
...
def _win_endpoint():
    if _volume_backend is not None:
        return _volume_backend
    _ensure_com()
    ...
    class _Pycaw:
        def __init__(self):
            ...
            # Current pycaw returns AudioDevice — use EndpointVolume, not Activate.
        ...
        def set_scalar(self, value: float) -> None:
            self._vol.SetMasterVolumeLevelScalar(max(0.0, min(1.0, float(value))), None)
        ...
        def snap_percent(self, percent: int) -> None:
            """Land on the mixer tick for this percent (device steps, not analog)."""
            try:
                step, count = self._vol.GetVolumeStepInfo()
                if not count or count < 2:
                    return
                want = max(0, min(int(count) - 1, round(percent / 100.0 * (int(count) - 1))))
                cur = int(step)
                guard = 0
                while cur < want and guard < int(count):
                    self._vol.VolumeStepUp(None)
                    cur, _ = self._vol.GetVolumeStepInfo()
                    cur = int(cur)
                    guard += 1
                while cur > want and guard < int(count) * 2:
                    self._vol.VolumeStepDown(None)
                    cur, _ = self._vol.GetVolumeStepInfo()
                    cur = int(cur)
                    guard += 1
            except Exception:
...
def _volume_step(step, default: int = 10) -> int:
    parsed = _parse_volume_value(step, default=None)
    if parsed is None:
        return default
...
def volume_up(step: int = 10):
    step = _volume_step(step)
    current = volume_get()
    if current is not None:
        volume_set(min(100, current + step))
        return
    if _OS == "Windows":
        _press_vk(_VK_VOLUME_UP, max(1, round(step / 2)))
    elif _OS == "Darwin":
...
def volume_down(step: int = 10):
    ...
    current = volume_get()
    if current is not None:
        volume_set(max(0, current - step))
        return
    if _OS == "Windows":
        _press_vk(_VK_VOLUME_DOWN, max(1, round(step / 2)))
    elif _OS == "Darwin":
        subprocess.run(["osascript", "-e",
            f"set volume output volume (output volume of (get volume settings) - {step})"],
            capture_output=True)
...
def volume_mute():
    if _OS == "Windows":
        try:
            _win_endpoint().set_mute(True)
            return
        except Exception as e:
            print(f"[Settings] mute via pycaw failed: {e}")
            _press_vk(_VK_VOLUME_MUTE)
            return
    if _OS == "Darwin":
...
def volume_unmute():
    if _OS == "Windows":
        try:
            ...
            print(f"[Settings] unmute via pycaw failed: {e}")
            _press_vk(_VK_VOLUME_MUTE)
            return
    if _OS == "Darwin":
...
def volume_toggle_mute():
    if _OS == "Windows":
        try:
            ep = _win_endpoint()
            ep.set_mute(not ep.get_mute())
            return
        except Exception as e:
            print(f"[Settings] toggle mute via pycaw failed: {e}")
            _press_vk(_VK_VOLUME_MUTE)
            return
    volume_mute() if _OS != "Linux" else subprocess.run(
...
def volume_get() -> int | None:
    ...
    try:
        if _OS == "Windows":
            # Scalar 0–1 matches the Windows volume slider. The old dB
            # conversion (10^(dB/20)) did not, so "set to 50%" landed nowhere
            # near 50% and up/down via pyautogui media keys often did nothing.
            return max(0, min(100, round(_win_endpoint().get_scalar() * 100)))
        if _OS == "Darwin":
...
def volume_set(value: int):
    parsed = _parse_volume_value(value, default=None)
    if parsed is None:
    ...
    if _OS == "Windows":
        try:
            ep = _win_endpoint()
            target = value / 100.0
            ep.set_scalar(target)
            if value > 0:
                ep.set_mute(False)
            if hasattr(ep, "snap_percent"):
                ep.snap_percent(value)
            got = round(float(ep.get_scalar()) * 100)
            if got != value:
                ep.set_scalar(target)
                if hasattr(ep, "snap_percent"):
                    ep.snap_percent(value)
            ...
            if current is None:
                current = 50
            delta = value - current
            # Windows volume keys are ~2% per press.
            times = max(1, min(50, round(abs(delta) / 2)))
...
_IRREVERSIBLE = {
    ...
    "toggle_wifi": ("Switch WiFi off or on",
                    "If this switches WiFi off, FRIDAY loses its connection and "
...
def computer_settings(
    ...
    if _looks_like_volume(action, description):
        action, value = _resolve_volume_intent(action, description, value)

    if not action:
        return _suggest(description or raw_action)

    if not _PYAUTOGUI and action not in _VOLUME_ACTIONS:
    ...
    if action == "volume_set":
        try:
            target = _parse_volume_value(value, default=None)
            if target is None:
                target = _parse_volume_value(description, default=None)
            if target is None:
                return (
                    "I need a volume from 0 to 100. "
                    "Say for example: set volume to 30."
                )
            before = volume_get()
            volume_set(target)
            after = volume_get()
            ...
            shown = after if after is not None else target
            if shown != target:
                return f"Volume set to {shown}% (closest the device allows to {target}%)."
            return f"Volume set to {shown}%."
        except Exception as e:
            return f"Could not set volume: {e}"

    if action in ("volume_up", "volume_down"):
        step = _parse_volume_value(value, default=None) or 10
        before = volume_get()
        try:
            if action == "volume_up":
                volume_up(step)
        ...
        if before is not None:
            push_undo(f"volume ({action})",
                      lambda b=before: (volume_set(b), f"Volume back to {b}%.")[1])
