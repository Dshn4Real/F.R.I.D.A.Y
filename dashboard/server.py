...
def _ensure_certs() -> bool:
    ...
    key_p = certs / "FRIDAY.key"
    crt_p = certs / "FRIDAY.crt"
    ...
    try:
        ...
        who = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "FRIDAY Dashboard"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "FRIDAY"),
        ])
        ...
        try:
            from core.secure_store import seal_file
            seal_file(key_p)
        except Exception:
            pass
...
<p>Connecting to FRIDAY…</p>
        ...
        async def command(req: Request):
            ...
            if enc:
                ...
                return JSONResponse(
                    {"error": "Encrypted payload required"}, status_code=400
                )
            if text:
    ...
    async def _serve_alias(self) -> None:
        ...
        keyfile  = getattr(self, "_tls_keyfile", str(ssl_key))
        asyncio.get_event_loop().run_in_executor(None, _ensure_network_access, PORT + 1)
        cfg = uvicorn.Config(
            self.app, host="0.0.0.0", port=PORT + 1, log_level="warning",
            ssl_keyfile=keyfile, ssl_certfile=str(ssl_cert),
        )
    ...
    async def serve(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "FRIDAY.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "FRIDAY.crt"
        ssl_key_path = str(ssl_key)
        if use_ssl:
            try:
                from core.secure_store import materialize_plaintext, seal_file
                seal_file(ssl_key)
                ssl_key_path = str(materialize_plaintext(ssl_key))
            except Exception as e:
                print(f"[Dashboard] Could not unwrap TLS key ({e}) — using file as-is.")
        self._tls_keyfile = ssl_key_path
        ...
        cfg = uvicorn.Config(
            ...
            **({"ssl_keyfile": ssl_key_path, "ssl_certfile": str(ssl_cert)} if use_ssl else {}),
        )
